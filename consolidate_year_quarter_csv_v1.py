#!/usr/bin/env python3
"""
Script para consolidar archivos CSV mensuales de XAUUSD por trimestre y convertirlos a Parquet.
...
def setup_logging(log_dir, log_level=logging.INFO):
    ...
    log_file = log_dir / f"consolidar_trimestre_{timestamp}.log"
...
def save_to_parquet(df, output_dir, year, quarter, logger, compression='snappy'):
    """
...
def generate_summary(df, filepath, logger):
    ...
    logger.info(f"Días en el trimestre: {(df['timestamp'].max() - df['timestamp'].min()).days + 1}")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Consolida archivos CSV mensuales de XAUUSD por trimestre y los convierte a Parquet',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Ejemplos:
  %(prog)s --year 2019 --quarter 1 --input_dir ../DATOS_ORIGEN/data_2019 --output_dir ../DATOS_ORIGEN/consolidated_quarter
  %(prog)s --year 2019 --quarter 2 --input_dir ../DATOS_ORIGEN/data_2019 --output_dir ../DATOS_ORIGEN/consolidated_quarter --log_level DEBUG
        """
    )
    ...
    # Log de inicio
    logger.info("="*60)
    logger.info("INICIANDO CONSOLIDACIÓN DE TRIMESTRE")
    logger.info("="*60)
    logger.info(f"Año: {args.year}")
    logger.info(f"Trimestre: Q{args.quarter}")
    ...
    try:
        ...
        if missing_files:
            ...
            if len(missing_files) == len(months):
                logger.error("No se encontró ningún archivo CSV para el trimestre")
                sys.exit(1)
        ...
        # Guardar a Parquet
        output_file = save_to_parquet(df, args.output_dir, args.year, args.quarter, logger, args.compression)
